...
func TestParserSuccess(t *testing.T) {
	...
	msg, err = Parse([]byte(`[2,42,"temperature",[28.7]]`))
	assert.NoError(t, err)
	assert.Equal(t,
		&Msg{
			Type:   Event,
...
func TestParserErrorsType(t *testing.T) {
	msg, err := Parse([]byte(`[3,42,"ping",[]]`))
	assert.EqualError(t, err, "message type must be 0, 1 or 2")
	assert.Nil(t, msg)

	msg, err = Parse([]byte(`[0.1,42,"pong",[]]`))
	assert.EqualError(t, err, "failed to parse type: expected unsigned integer got: float")
	assert.Nil(t, msg)

	msg, err = Parse([]byte(`["0",42,"pong",[]]`))
...
func TestParserErrorsRequestID(t *testing.T) {
	...
	msg, err = Parse([]byte(`[0,42.1,"ping",[]]`))
